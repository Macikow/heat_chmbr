...
char END_OF_TEMP[] = "C]";
char SET_TEMP[] = "SETPOINT: [";
char COUNTDOWN[] = "COUNTDOWN T[";
char END_TIME[] = "TIME END, PRESS OK";
char EXIT[]	= "EXIT, ARE YOU SURE ?";
...
void ui_refresh_memory_data()
{
	rom_refresh();
	mem1Time.hours = rom_get_time_hours(ROM_MEM1);
	mem1Time.minutes = rom_get_time_minutes(ROM_MEM1);
	mem1Temp.temp = rom_get_temp(ROM_MEM1);

	mem2Time.hours = rom_get_time_hours(ROM_MEM2);
	mem2Time.minutes = rom_get_time_minutes(ROM_MEM2);
	mem2Temp.temp = rom_get_temp(ROM_MEM2);

	mem3Time.hours = rom_get_time_hours(ROM_MEM3);
	mem3Time.minutes = rom_get_time_minutes(ROM_MEM3);
	mem3Temp.temp = rom_get_temp(ROM_MEM3);
}
...
{
	tc_set_counter(progTime.hours, progTime.minutes);
	tc_set_mode(TC_MODE_NORMAL_PROGRAM);
	rom_wite_new_set_data(progTime.hours, progTime.minutes , progTemp.temp);
	ui_refresh_memory_data();
	tc_run_downcounter(); // TODO usunąć implementacja powinna być w innym miejscu
	button_event_handler = pm_event;
	program_mode_handler(pm_event_button_ok);
}
...
void mem1_callback()
{
	tc_set_counter(mem1Time.hours, mem1Time.minutes);
	tc_set_mode(TC_MODE_MEMORY);
	tc_run_downcounter(); // TODO usunąć implementacja powinna być w innym miejscu
	button_event_handler = pm_event;
	program_mode_handler(pm_event_button_ok);
}
void mem2_callback()
{
	tc_set_counter(mem2Time.hours, mem2Time.minutes);
	tc_set_mode(TC_MODE_MEMORY);
	rom_mem2_exec();
	ui_refresh_memory_data();
	currentPointer = (menu_t *)&mem1Start;
	lcd_row_pos_level_2 = 0;
	// TODO pozamieniać kolejności menu - exit powinien przenieść nas do mem1
	tc_run_downcounter(); // TODO usunąć implementacja powinna być w innym miejscu
	button_event_handler = pm_event;
	program_mode_handler(pm_event_button_ok);
}
void mem3_callback()
{
	tc_set_counter(mem3Time.hours, mem3Time.minutes);
	tc_set_mode(TC_MODE_MEMORY);
	rom_mem3_exec();
	ui_refresh_memory_data();
	currentPointer = (menu_t *)&mem1Start;// TODO pozamieniać kolejności menu - exit powinien przenieść nas do mem1
	lcd_row_pos_level_2 = 0;
	tc_run_downcounter(); // TODO usunąć implementacja powinna być w innym miejscu
	button_event_handler = pm_event;
	program_mode_handler(pm_event_button_ok);
...
void program_event(uint8_t button)
{
	switch(button)
	{
		...
		case UI_BUTTON_OK:
			...
			break;
		case UI_BUTTON_BACK:
			program_mode_handler(pm_event_button_exit);
			break;
	}
}
...
void convert_downcount_timer_to_str(uint8_t time_h, uint8_t time_m, uint8_t sec)
{
	char zero_str = 48;
	time_string[0] = '[';
	time_string[1] = time_h/10 + zero_str;
...
void iu_time_handler(uint8_t button_hold)
{
  ...
  if(((ms_handler_counter % 986 ) == 0) && (ms_handler_counter != 0))
  {
	  if(button_event_handler == heating_event) infinite_heater_handler(ih_event_time_1s);
	  if(button_event_handler == pm_event) program_mode_handler(pm_event_time_1s);
  }
  if(((s_handler_counter % 7) == 0) && (ms_handler_counter == 0))
  {
	  if(button_event_handler == heating_event) infinite_heater_handler(ih_event_time_7s);
	  if(button_event_handler == pm_event) program_mode_handler(pm_event_time_7s);
  }
}
...
{
	build_menu(&heat,"INFINITE MODE",		&prog,	NULL,	&htemp,		NULL,	NULL,0,25.0f,0,0);
		build_menu(&htemp,"SET TEMP",			&hstart,NULL,	NULL,	&heat,	ui_set_temp_callback,SET_OPTION_TEMP,25.0f,0,0);
		build_menu(&hstart,"START INFINITE M.",		NULL,	&htemp,	NULL,	&heat,	&infinite_heating_callback,0,25.0f,0,0);
	...
	build_menu(&mem,"PROGRAM MEM",	&params,&seq,&mem1,NULL,	NULL,0,25.0f,0,0);
		build_menu(&mem1,"PROGRAM MEM1",&mem2,NULL,&mem1Time,	&mem,NULL,0,25.0f,0,0);
			build_menu(&mem1Time,"TIME MEM1",		&mem1Temp, NULL,		 NULL,&mem1,NULL,SET_OPTION_TIME,mem1Temp.temp,mem1Time.hours,mem1Time.minutes);
			build_menu(&mem1Temp,"TEMP MEM1",		&mem1Start,&mem1Time,NULL,&mem1,NULL,SET_OPTION_TEMP,mem1Temp.temp,mem1Time.hours,mem1Time.minutes);
			build_menu(&mem1Start,"START MEM1",	NULL,			 &mem1Temp,NULL,&mem1,&mem1_callback,0,mem1Temp.temp,mem1Time.hours,mem1Time.minutes);
		build_menu(&mem2,"PROGRAM MEM2",&mem3,&mem1,&mem2Time,&mem,NULL,0,25.0f,0,0);
			build_menu(&mem2Time,"TIME MEM2",  &mem2Temp, NULL,			NULL,&mem2,NULL,SET_OPTION_TIME,mem2Temp.temp,mem2Time.hours,mem2Time.minutes);
			build_menu(&mem2Temp,"TEMP MEM2",	 &mem2Start,&mem2Time,NULL,&mem2,NULL,SET_OPTION_TEMP,mem2Temp.temp,mem2Time.hours,mem2Time.minutes);
			build_menu(&mem2Start,"START MEM2",NULL,			&mem2Temp,NULL,&mem2,&mem2_callback,0,mem2Temp.temp,mem2Time.hours,mem2Time.minutes);
		build_menu(&mem3,"PROGRAM MEM3",NULL,&mem2,&mem3Time,&mem,NULL,0,25.0f,0,0);
			build_menu(&mem3Time,"TIME MEM3",		&mem3Temp,	NULL,			NULL,	&mem3,NULL,SET_OPTION_TIME,mem3Temp.temp,mem3Time.hours,mem3Time.minutes);
			build_menu(&mem3Temp,"TEMP MEM3",		&mem3Start,	&mem3Time,NULL,	&mem3,NULL,SET_OPTION_TEMP,mem3Temp.temp,mem3Time.hours,mem3Time.minutes);
			build_menu(&mem3Start,"START MEM3",	NULL,				&mem3Temp,NULL,	&mem3,&mem3_callback,0,mem3Temp.temp,mem3Time.hours,mem3Time.minutes);
